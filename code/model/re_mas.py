...
class MAS(nn.Module):
    ...
    def forward(self, x_BxSxT, y_true_BxC=None, step=None):
        B, S, T = x_BxSxT.size()
        # print('current data dimension is %s start a new batch bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb' \
        # % str((B, S, T)))
        ...
        # print('word dimension transfer back to B, S, T size got, Also another format of Zt %s'\
         # % str(p_BxSxTxC.size()))

        # word-level representation/value
        # print('word-level dimension tanh transfer to value got, this step is getting the key for \
        # later calculation %s' % str(z_BSxTxD.size()))

        # z_key_BSxTxE = torch.tanh(self.word_transform(x_BSxTxD)).chunk(H, -1)
        z_key_BSxTxE = relu(self.word_transform(x_BSxTxD)).chunk(H, -1)

        # print('first dimension chunk transfer got 12 heads of chunks, \
        # last dimension/12, this is the key in paper, it has a tanh and linear process %s, \
        # the size is %s' % (str(np.shape(z_list_BSxTxE[0])), str(len(z_list_BSxTxE))))
        z_qry_list_E = self.word_key.chunk(H, -1)
        # print('dimension word_key chunk transfer got, function is nn.Parameter.chunk \
        # this is the query in paper, this vector has nothing to do with the data %s, \
        # the size is %s' % (str(np.shape(z_key_list_E[0])), str(len(z_key_list_E))))


        s_list_BxSxC = [] # the container takes zzzzzzhhhhhhh
        h_list_BxSxE = [] # the container takes zt element wise times head attention ah, which means\
         # the key followed the steps in the model
        ...
        for z_BSxTxE, z_qry_E in zip(z_key_BSxTxE, z_qry_list_E):
            ...
            # if mini_step <= 1:
            #     print('\n')
                # print('this step query z_key_E %s times key z_BSxTxE %s get aaaaaahhhhhh a_BSxT %s' \
                # %(str(np.shape(z_key_E)), str(np.shape(z_BSxTxE)), str(np.shape(a_BSxT))))
            ...
            # if mini_step < 1:
            #     print('\n')
            #     print('after softmax each attention ah size is: %s, ah appened to a list p_wt_list_BxSxT \
            # size is: %s, each element inside is %s' % (str(a_BSxT.size()),str(np.shape(p_wt_list_BxSxT)), \
            # str(a_BSxT.view(B, S, T).size())))
            ...
            # -------------------------------------------')
            #     print('sum up the product of Zt %s and ah %s got q_BSxC which is zzzzzzzzhhhhhhhhhhh \
            # %s' % (str(p_BSxTxC.size()), str(a_BSxT.unsqueeze(-1).size()), str(q_BSxC.size())))
            s_list_BxSxC.append(s_BSxC.view(B, S, -1))
            # if mini_step < 1:
            #     print('append all the ZZZZZZZZZZZZZZZZZZZZhhhhhhhhhhhhhhhhhh to a list %s, \
            # inside the list each head prediction with size %s' % (len(q_list_BxSxC), str(q_list_BxSxC[0].size())))
            h_BSxE = torch.sum(z_BSxTxE * a_BSxT.unsqueeze(-1), 1)
            # if mini_step < 1:
                # print('\n')
                # print('Get encodings of sentence-level %s, its a sumup of key z_BSxTxE %s and ah \
                # last dimension unsqueeze %s' % (str(h_BSxE.size()), str(z_BSxTxE.size()), \
                # str(a_BSxT.unsqueeze(-1).size())))
                # print('this step perform a token-level likely encoding got h_BSxE size is: %s' % \
                # str(h_BSxE.size()))
            h_list_BxSxE.append(h_BSxE.view(B, S, -1))
            # if mini_step < 1:
            #     print('this step append all word query that has same calculation procedure \
            # got h_list_BxSxE size is: %s, one of the query wirh size %s' % (str(np.shape(h_list_BxSxE)), \
            # str(h_list_BxSxE[0].size())))
            # mini_step += 1
        # print('after the attention transfer, each updated sentence level key was calculated: %s' % \
        # str(np.shape(h_list_BxSxE)))
        # print('append all the ZZZZZZZZZZZZZZZZZZZZhhhhhhhhhhhhhhhhhh to a list %s, \
        # inside the list each head prediction with size %s' % (len(q_list_BxSxC), str(q_list_BxSxC[0].size())))
        # print('This step stack all the attention heads prediction Zh, : %s the length of this \
        # list is %s' % (str(q_BxSxHxC.size()), str(len(q_list_BxSxC))), q_BxSxHxC)
        s_BxSxC = torch.stack(s_list_BxSxC, -2).max(dim=-2)[0]
        # print('This step get max of all the attention heads prediction Zh which is the max-pooling \
        # step: ZZZZZZZZZZZZZZZZZSSSSSSSSSSSSSSSS get %s' % str(q_BxSxC.size()), q_BxSxC)
        p_wt_BxSxT = torch.stack(p_wt_list_BxSxT, -2).max(-2)[0]
        # print('this p_wt_BxSxT stack all the attention in a sentence toghther which has a size %s' \
        # % str(p_wt_BxSxT.size()))
        h_BxSxHxE = torch.stack(h_list_BxSxE, -2)
        # print('this h_BxSxHxE stack all the Zh in a sentence toghther which has a size %s' % \
        # str(h_BxSxHxE.size()))
        h_BxSxD = h_BxSxHxE.view(B, S, D)
        # print('this h_BxSxD stack all the h_BxSxHxE in a sentence toghther get sentence level \
        # key which has a size %s' % str(h_BxSxD.size()))
        ...
        #             miemie.append('True')
        #     print('how many aspect has not been detected at all from the results \
        # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<  %d' % len(miemie))
        #     mini_step += 1
