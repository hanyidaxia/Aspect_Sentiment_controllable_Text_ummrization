...
def create_aspect_dev(args,
                       model_name):

    files = os.listdir(args.data_dir + args.dataset + '/' + model_name + '/')
    print(files)
    dtype_ = 'float64'
    for file in files:
        data = []
        f = open(args.data_dir + args.dataset + '/' + model_name + '/' + file, 'r')
        for line in tqdm(f):
            inst = json.loads(line.strip())
            data.append(inst)
        f.close()
        ...
        for inst in data:
            tmp_list = []
            if max(inst['switch'][:7]) < 0.002:
                tmp_switch = np.zeros_like(inst['switch'][:7])
                tmp_switch[:7][random.randint(0, 6)] = 1
                inst['switch'] = [int(i) for i in list(tmp_switch)] + inst['switch'][7:]
            if np.any(len([1 for x in inst['switch'] if x > 0]) == 2):
                tmp_switch = np.zeros_like(inst['switch'][:7])
                tmp_switch[np.argmax(inst['switch'][:7])] = 1
                inst['switch'] = [int(i) for i in list(tmp_switch)] + inst['switch'][7:]
            elif np.any(len([1 for x in inst['switch'] if x > 0]) == 1):
                inst['switch'][:7][random.randint(0, 6)] = 1
            elif np.any(len([1 for x in inst['switch'][:7] if x > 0]) > 1):
                if np.any(len([1 for x in inst['switch'][:5] if x > 0]) > 0):
                    tmp_switch = np.zeros_like(inst['switch'][:5])
                    tmp_switch[np.argmax(inst['switch'][:5])] = 1
                    inst['switch'] = [int(i) for i in list(tmp_switch)] + [0, 0] + inst['switch'][7:]
                else:
                    tmp_switch = np.zeros_like(inst['switch'][:7])
                    tmp_switch[np.argmax(inst['switch'][:7])] = 1
                    inst['switch'] = [int(i) for i in list(tmp_switch)] + inst['switch'][7:]

        with open (args.data_dir + args.dataset + '/' + model_name + '/' + 're' + file, 'w') as f:
            for i in data:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-dataset', default='allCampusmin60max120summary_source_num5review_source_num60keywords5',
    type=str)
...
type=str)
    ...
    if args.mode == 'train':
        ...
        create_aspect_dev(args, 'Sentiment_filtered10_BCE_Sum_weightstep.10000 \
average_loss.613.18 sentence_f1.79.83 document_f1.81.44_best_dataBCE_Sum_weight16000')
