...
from re_mas import MAS
import random
sys.path.append(os.path.realpath('..'))
from data_processor.re_readers import AspectDetectionDataset, aspect_detection_collate
"""
...
def train(args):
    ...
    dev_dataset = AspectDetectionDataset(args.data_dir + '/' + args.dataset + '/' + args.dev_file,
    ...
    while step < args.no_train_steps:
        ...
        for inp_batch, out_batch in tqdm(dataloader):
            ...
            if step % args.check_every == 0:
                print('Step SSSTTTTTTEEEEEEEEEEEEEEEEEEEPPPPPPPPPPPPPPPPPPPPPPPPP \
                %d Train LossSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS %.4f' % (step, np.mean(losses)))
                    ...
                    for bid in range(len(out_batch)):
                        for aid in range(args.num_aspects):
                            ...
                            for sid in range(len(sentence_pred[bid])):
                                _update_counts(out_batch[bid][aid], sentence_pred[bid][sid][aid],
                                sent_counts[aid])
                    loss = preds['loss']
                ...
                rand_idx = random.randint(0, args.batch_size-52)
                inp = inp_batch[rand_idx]
                print('Document prediction is %s and the Gold label was %s'%
                (str(document_pred[rand_idx].tolist()), str(out_batch[rand_idx].tolist())))
            ...
            if step % args.ckpt_every == 0:
                print('Saving...')
                os.makedirs(args.model_dir + '/' + args.dataset + '/' + args.train_file.split('.')[0] \
                + args.model_name + str(args.no_train_steps), exist_ok=True)
                torch.save({
                ...
                }, args.model_dir + '/' + args.dataset + '/' + args.train_file.split('.')[0] + args.model_name \
                + str(args.no_train_steps) + '/' \
                + args.model_name + 'step.%d average_loss.%.2f sentence_f1.%.2f document_f1.%.2f' % \
                (step, np.mean(losses), doc_f1, sent_f1))
                losses = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('-train_file', default='re_best.jsonl', type=str)
    parser.add_argument('-dev_file', default='redev_best.jsonl', type=str)
    ...
    parser.add_argument('-no_train_steps', default=30000, type=int)
    parser.add_argument('-no_warmup_steps', default=15000, type=int)
    parser.add_argument('-check_every', default=100, type=int)
    parser.add_argument('-ckpt_every', default=1000, type=int)
    parser.add_argument('-attribute_lexicon_name', default='/home/jade/untextsum/\
    Data/20220111_attribute_lexicon.json', type = str)
